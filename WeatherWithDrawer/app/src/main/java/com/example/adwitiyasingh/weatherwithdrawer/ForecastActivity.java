...
public class ForecastActivity extends AppCompatActivity {
    ...
    ForecastModel getJson(String json) {
        ...
        try {
            ...
            JSONObject city = thisJsonObject.getJSONObject("city");
            JSONObject coord = city.getJSONObject("coord");
            org.json.JSONArray list = thisJsonObject.getJSONArray("list");

            com.example.adwitiyasingh.weatherwithdrawer.model.Coord c = new model.Coord(coord.getDouble("lat"), coord.getDouble("lon"));
            ...
            for (int i = 0; i < list.length(); i++) {
                ...
                JSONObject man = now.getJSONObject("main");
                JSONObject wnd = now.getJSONObject("wind");
                String dttime = now.getString("dt_txt");
                com.example.adwitiyasingh.weatherwithdrawer.model.Weather wdrob = new model.Weather(wtr0.getInt("id"), wtr0.getString("main"), wtr0.getString("description"), wtr0.getString("icon"));
                com.example.adwitiyasingh.weatherwithdrawer.model.Main mnob = new model.Main(man.getInt("humidity"), man.getInt("pressure"), man.getDouble("temp"), man.getDouble("temp_max"), man.getDouble("temp_min"));
                com.example.adwitiyasingh.weatherwithdrawer.model.Wind wndob = new model.Wind(wnd.getDouble("speed"), wnd.getInt("deg"));
